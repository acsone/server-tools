...
class AutoExport(models.Model):
    ...
    @api.model
    def _get_default_is_change_user_id_allowed(self):
        return self._get_default_user_id().id == SUPERUSER_ID
    ...
    is_change_user_id_allowed = fields.Boolean(
        compute='_compute_is_change_user_id_allowed',
        store=False,
        readonly=True,
        default=lambda self: self._get_default_is_change_user_id_allowed(),
    )
    ...
    def _inverse_user_id(self):
        if any(not rec.is_change_user_id_allowed for rec in self):
            raise ValidationError(_("You are not allowed to change the user."))

    @api.multi
    def _compute_is_change_user_id_allowed(self):
        """
        Only Admin can change the user field.
        """
        for rec in self:
            rec.is_change_user_id_allowed = self.env.user.id == SUPERUSER_ID
    ...
    def trigger_export(self):
        ...
        description = _(u"Asynchronous auto export")
        for rec in self:
            do_trigger_export.delay(
                session, rec._name, rec.id, description=description)
