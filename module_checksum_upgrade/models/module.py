...
class Module(models.Model):
    ...
    def upgrade_changed_checksum(self):
        ...
        process, this method returns False and an error is logged.
        It returns True only in case of complete success.
        """
        ...
        installed_modules = self.search([('state', '=', 'installed')])
        modules_changed = installed_modules.filtered(
            lambda r: r._get_checksum_dir() != saved_checksums.get(r.name),
        )
        partial_modules = self.search([
            '!',
            ('state', 'in', ('installed', 'uninstalled', 'uninstallable')),
        ])
        if not modules_changed and not partial_modules:
        ...
        _logger.info("Marking the following module to upgrade "
                     ...
                     ','.join(modules_changed.mapped('name')))
        modules_changed.button_upgrade()
        ...
        if partial_modules:
            _logger.error("Upgrade not complete for the following "
                          "modules: %s",
