...
class TestGeoJsonifyExport(TestJsonifyMixin):
    ...
    def test_export(self):
        ...
        # when
        self.user_2.name = "New name"  # not part of the export
    ...
    def test_create_write_unlink_export_line(self):
        ...
        user_line.name = "user_id/login"

        # then  # everything needs a recompute
        self.assertTrue(all(self.records.mapped("jsonify_data_todo")))
        # and we modified the model's depends
        field_depends = self.record_2._fields["jsonify_data_todo"].depends
        ...
        # when
        self.user_2.login = "newlogin"  # it's now part of the export
