...
class TestFloatNullable(TransactionCase):
    ...
    def test_null_handling(self):
        ...
        self.assertTrue(
            self._get_db_storge_value(self.record_empty, "value_float") == 0.0
        )
    ...
    def test_zero_handling(self):
        ...
        # check db storage value
        self.assertTrue(
            self._get_db_storge_value(self.record_zero, "value_float_nullable") == 0.0
        )
        with self.assertRaises(AssertionError):
            self.assertIsNone(
                self._get_db_storge_value(self.record_zero, "value_float_nullable")
            )
        with self.assertRaises(AssertionError):
            self.assertIsNone(
                self._get_db_storge_value(self.record_zero, "value_float")
            )
        self.assertTrue(
            self._get_db_storge_value(self.record_zero, "value_float") == 0.0
        )
