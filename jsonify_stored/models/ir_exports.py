...
class IrExport(models.Model):
    ...
    def _check_jsonify_stored(self):
        ...
        for model in models:
            recompute_field = model._fields["jsonify_data_todo"]
            recompute_field.depends = model._jsonify_get_export_depends()
            recompute_field.setup_triggers(model)
            model.search([]).write({"jsonify_data_todo": True})
        self.env.registry.invalidated = True
